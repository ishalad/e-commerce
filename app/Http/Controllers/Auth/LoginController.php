...
class LoginController extends Controller
{
    ...
    */
    ...
    public function authenticated()
    {
        ...
        } elseif (auth()->user()->user_type == 'super_dealer') {
            if(auth()->user()->dealer_status == 1) {
                return redirect()->route('superdealer.dashboard');
            } elseif (empty(auth()->user()->phone_verified_at) || empty(auth()->user()->email_verified_at)) {
                $user = auth()->user();
                $this->guard()->logout();
                return redirect()->route('phone',[encrypt($user->id)]);
            } else {
                $this->guard()->logout();
                flash(translate("Once admin approved your account, then you would be able to log in."))->warning();
                return redirect()->route('user.login');
            }
