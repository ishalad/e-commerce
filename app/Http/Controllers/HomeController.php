...
class HomeController extends Controller
{
    ...
    {
        $user = Auth::user();
        return view('frontend.user.gst_details', compact('user'));
    }
    ...
    public function phone_verify(Request $request)
    {
        ...
        if ($user) {
            $response['status'] = 2;
            $response['message'] = translate('Phone already exists!');
        } else {
            $user = Auth::user();
            $user->phone = $request->phone;
            ...
            $user->save();
            $response['status'] = 1;
            $response['message'] = translate("Your verification code has been sent to your phone.");
        }
        return json_encode($response);
    }
    ...
    public function phone_code(Request $request)
    {
        $user = Auth::user();
        if ($user) {
            if (empty($user->phone) && empty($user->phone_verification_otp)) {
                flash(translate('First verify your phone.'))->warning();
                return back();
            } else {
                $phoneotp = $user->phone_verification_otp;
                return view("seller.profile.phone_code", compact('phoneotp'));
            }
        } else {
            flash(translate('User not found.'))->warning();
            return back();
        }
    }

    public function phone_code_data(Request $request)
    {
        if (strlen($request->phone_code) == 6) {
            $user = Auth::user();
            if ($user) {
                if ($user->phone_verification_otp == $request->phone_code) {
                    $user->phone_verified_at = Carbon::now();
                    $user->save();
                    if ($user->is_profile == 1 && $user->shop->verification_status == 1) {
                        return redirect()->route('seller.dashboard');
                    } else {
                        flash(translate('Your phone has verify successfully.'))->success();
                        return redirect()->route('seller.profile.index');
                    }
                } else {
                    flash(translate('Code is not match.'))->warning();
                    return view("seller.profile.phone_code");
                }
            } else {
                flash(translate('User not found.'))->warning();
                return view("seller.profile.phone_code");
            ...
            flash(translate('Enter only 6 digits.'))->warning();
            return view("seller.profile.phone_code");
        }
    }
    ...
    {
        if ($request->gst_status == "1") {
            $validator = Validator::make($request->all(), [
                'gst_document' => 'required',
                'gst_number' => 'required',
                'gst_business_name' => 'required',
            ]);
            if ($validator->fails()) {
            ...
            if (!empty($request->gst_number)) {
                $res = $response = "";
                $api_url = "http://sheet.gstincheck.co.in/check/" . env('GST_API_KEY') . "/" . $request->gst_number;
                $response = Http::get($api_url);

                if (!empty($response)) {
                    $res = $response->body();
                } else {
                    flash(translate("Invalid GST Number."))->warning();
                    return back();
                }

                //$res = '{"flag":true,"message":"GSTIN  found.","data":{"ntcrbs":"SPO","adhrVFlag":"Yes","lgnm":"MONISH BEHURA","stj":"State - Gujarat,Division - Division - 7,Range - Range - 15,Unit - Ghatak 57 (Surat)","dty":"Regular","cxdt":"","gstin":"24AZHPB2360C1Z3","nba":["Supplier of Services"],"ekycVFlag":"Not Applicable","cmpRt":"NA","rgdt":"27/01/2022","ctb":"Proprietorship","pradr":{"adr":"Ground Floor, Shop No.13, RAJ ARCADE, TP 14, LNM.Opp. Raj Victoria, Near Pal Walk Way, Pal, Adajan, Surat, Surat, Gujarat, 395009","addr":{"flno":"","lg":"","loc":"Surat","pncd":"395009","bnm":"Pal","city":"","lt":"","stcd":"Gujarat","bno":"0","dst":"Surat","st":"Adajan"}},"sts":"Active","tradeNam":"CHAI SUTTA BAR","isFieldVisitConducted":"No","adhrVdt":"11/02/2022","ctj":"State - CBIC,Zone - VADODARA,Commissionerate - SURAT,Division - DIVISION-II SURAT,Range - RANGE-V (Jurisdictional Office)","einvoiceStatus":"No","lstupdt":"","adadr":[],"ctjCd":"","errorMsg":null,"stjCd":""}}';

                if (!empty($res)) {
                    $res_array = json_decode($res);
                    if ($res_array->flag) {
                        if ($res_array->data->tradeNam == strtoupper($request->gst_business_name)) {
                            if (Auth::user()->user_type == "customer") {
                                $user = User::where('id', Auth::user()->id)->first();
                                $user->gst_status = 1;
                                $user->gst_number = $request->gst_number;
                                $user->gst_document = $request->gst_document;
                                $user->verification_gst = 1;
                                $user->save();
                            } else {
                                $shop = Shop::where('user_id', Auth::user()->id)->first();
                                $shop->gst_status = 1;
                                $shop->gst_number = $request->gst_number;
                                $shop->gst_document = $request->gst_document;
                                $shop->verification_gst = 1;
                                $shop->save();
                            }
                            flash(translate("GST number has been successfully verified."))->success();
                            return back();
                        } else {
                            flash(translate("Business name is not match with GST number."))->warning();
                            return back();
                        }
                    } else {
                        flash(translate("Invalid GST Number."))->warning();
            ...
            if (Auth::user()->user_type == "customer") {
                $user = User::where('id', Auth::user()->id)->first();
                $user->gst_status = 2;
                $user->gst_number = $request->enrollment_number;
                $user->gst_document = $request->enrollment_document;
                $user->verification_gst = 1;
                $user->save();
            } else {
                $shop = Shop::where('user_id', Auth::user()->id)->first();
                $shop->gst_status = 2;
                $shop->gst_number = $request->enrollment_number;
                $shop->gst_document = $request->enrollment_document;
                $shop->verification_gst = 1;
                $shop->save();
            }
            flash(translate("Enrollment Number has been successfully saved."))->success();
            return back();
        }
    }
